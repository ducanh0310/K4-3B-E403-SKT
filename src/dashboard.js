...
export function renderDashboard(issues, {
  ...
  :root{color-scheme:dark;--bg:#090c13;--panel:#111827;--line:#263248;--text:#eef2ff;--muted:#94a3b8;--accent:#8b5cf6}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at top,#17203a 0,var(--bg) 42%);color:var(--text);font:15px/1.5 system-ui,-apple-system,Segoe UI,sans-serif}main{width:min(1180px,calc(100% - 32px));margin:auto;padding:36px 0}header{display:flex;justify-content:space-between;gap:24px;align-items:end;margin-bottom:24px}nav{display:flex;gap:8px;margin-top:14px}nav a{padding:7px 11px;border:1px solid var(--line);border-radius:999px;text-decoration:none}h1{margin:0;font-size:clamp(26px,4vw,42px);letter-spacing:-.04em}p{margin:6px 0 0;color:var(--muted)}.metrics{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:20px}.metric,.table{background:rgba(17,24,39,.88);border:1px solid var(--line);border-radius:16px;box-shadow:0 20px 60px rgba(0,0,0,.25)}.metric{padding:18px}.metric b{display:block;font-size:30px}.metric span,small,.muted{color:var(--muted)}.table{overflow:auto}table{width:100%;border-collapse:collapse;min-width:820px}th,td{text-align:left;padding:14px 16px;border-bottom:1px solid var(--line)}th{font-size:12px;text-transform:uppercase;letter-spacing:.08em;color:var(--muted)}tr:last-child td{border:0}small{display:block;font-family:ui-monospace,monospace;margin-top:3px}.badge{display:inline-block;padding:4px 9px;border-radius:999px;font-size:12px;font-weight:700;background:#334155}.badge.stuck{background:#7f1d1d;color:#fecaca}.badge.needs_review{background:#713f12;color:#fde68a}.badge.discussing{background:#164e63;color:#a5f3fc}.badge.open{background:#312e81;color:#c7d2fe}a{color:#c4b5fd;text-underline-offset:3px}.missing{color:#fca5a5;font-weight:650}.empty{padding:40px;text-align:center;color:var(--muted)}@media(max-width:680px){header{display:block}.metrics{grid-template-columns:1fr}main{padding-top:24px}}
  </style></head><body><main><header><div><p>${escapeHtml(eyebrow)}</p><h1>${escapeHtml(title)}</h1><p>Cập nhật ${escapeHtml(updated)} · tự làm mới sau 30 giây</p><nav aria-label="Chọn dữ liệu"><a href="/">Live</a><a href="/demo">K4 Dataset</a></nav></div></header><section class="metrics" aria-label="Tổng quan"><div class="metric"><b>${active.length}</b><span>issue đang mở</span></div><div class="metric"><b>${stuck}</b><span>issue bị kẹt</span></div><div class="metric"><b>${questions}</b><span>câu hỏi đang theo dõi</span></div></section><section class="table"><table><thead><tr><th>Trạng thái</th><th>Issue</th><th>Người hỏi</th><th>Số câu</th><th>Evidence</th><th>Hành động</th></tr></thead><tbody>${rows || '<tr><td colspan="6" class="empty">Không có issue đang mở.</td></tr>'}</tbody></table></section></main></body></html>`;
}
